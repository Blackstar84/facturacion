from django import forms

from .models import Categoria, SubCategoria, Marca, UnidadMedida, Producto


class CategoriaForm(forms.ModelForm):
    class Meta:
        model = Categoria
        fields = ['descripcion', 'estado']
        labels = {'descripcion': "Descripción de la Categoría",
                  'estado': "Estado"
                  }
        widget = {'descripcion': forms.TextInput()}

    def __init__(self, *args, **kwargs): #sobreescribimos el init del formulario
        super().__init__(*args, **kwargs) # invocamos el evento init del formulario de la clase padre
        for field in iter(self.fields): # le decimos que itere sobre todos los fields
            self.fields[field].widget.attrs.update({
                'class': 'form-control'
            })


class SubCategoriaForm(forms.ModelForm):
    categoria = forms.ModelChoiceField(
        queryset=Categoria.objects.filter(estado=True).order_by('descripcion')
        # Sobreescribimos el query set para que sólo traiga las categorías que tengan estado=True
    )
    class Meta:
        model = SubCategoria
        fields = ['categoria', 'descripcion', 'estado']
        labels = {'descripcion': "Sub Categoría",
                  'estado': "Estado"
                  }
        widget = {'descripcion': forms.TextInput()}

    def __init__(self, *args, **kwargs): #sobreescribimos el init del formulario
        super().__init__(*args, **kwargs) # invocamos el evento init del formulario de la clase padre
        for field in iter(self.fields): # le decimos que itere sobre todos los fields
            self.fields[field].widget.attrs.update({
                'class': 'form-control'
            })
        self.fields['categoria'].empty_label = "Seleccione Categoría"


class MarcaForm(forms.ModelForm):
    class Meta:
        model = Marca
        fields = ['descripcion', 'estado']
        labels = {'descripcion': "Descripción de la Marca",
                  'estado': "Estado"
                  }
        widget = {'descripcion': forms.TextInput()}

    def __init__(self, *args, **kwargs): #sobreescribimos el init del formulario
        super().__init__(*args, **kwargs) # invocamos el evento init del formulario de la clase padre
        for field in iter(self.fields): # le decimos que itere sobre todos los fields
            self.fields[field].widget.attrs.update({
                'class': 'form-control'
            })



class UMForm(forms.ModelForm):
    class Meta:
        model = UnidadMedida
        fields = ['descripcion', 'estado']
        labels = {'descripcion': "Descripción de la Unidad de Medida",
                  'estado': "Estado"
                  }
        widget = {'descripcion': forms.TextInput()}

    def __init__(self, *args, **kwargs): #sobreescribimos el init del formulario
        super().__init__(*args, **kwargs) # invocamos el evento init del formulario de la clase padre
        for field in iter(self.fields): # le decimos que itere sobre todos los fields
            self.fields[field].widget.attrs.update({
                'class': 'form-control'
            })            
            


class ProductoForm(forms.ModelForm):

    class Meta:
        model = Producto
        fields = ['codigo', 'codigo_barra', 'descripcion', \
                  'estado', 'precio', 'existencia', 'ultima_compra',\
                  'marca', 'subcategoria', 'unidad_medida']
        exclude = ['um', 'fm', 'uc', 'fc']
        
        widget = {'descripcion': forms.TextInput()}

    def __init__(self, *args, **kwargs): #sobreescribimos el init del formulario
        super().__init__(*args, **kwargs) # invocamos el evento init del formulario de la clase padre
        for field in iter(self.fields): # le decimos que itere sobre todos los fields
            self.fields[field].widget.attrs.update({
                'class': 'form-control'
            })
        self.fields['ultima_compra'].widget.attrs['readonly'] =True
        self.fields['existencia'].widget.attrs['readonly'] =True  